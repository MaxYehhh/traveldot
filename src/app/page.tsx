...
export default function Home() {
  ...
  if (loading) {
    return (
      <main className="flex items-center justify-center w-full h-screen bg-[#0a0a0c]">
  ...
  if (!session) {
    return (
      <main className="relative w-full h-screen flex items-center justify-center p-6 bg-[#0a0a0c]">
        <Auth />
      </main>
    );
  }
    ...
    <main className="relative w-full h-screen overflow-hidden">
      ...
      {fetchError && (
        <div className="absolute top-24 left-1/2 -translate-x-1/2 z-50 animate-in fade-in slide-in-from-top-4">
          <div className="flex items-center gap-2 px-4 py-2 bg-red-500/10 border border-red-500/20 text-red-200 rounded-full text-sm font-medium backdrop-blur-md">
            <AlertCircle size={16} />
            {fetchError}
          </div>
        </div>
      ...
      <div className="absolute top-6 left-6 z-10 flex flex-col gap-4 pointer-events-none">
        <div className={clsx(
          ...
          theme === 'dark' ? "text-white" : "bg-white/80 border-slate-200 shadow-xl"
        )}>
      ...
      <div className="absolute top-6 right-6 z-10 flex items-center gap-3">
        ...
        <button
          ...
          className={clsx(
            ...
            theme === 'dark' ? "hover:bg-rose-500/10 hover:border-rose-500/20 text-slate-300" : "bg-white/90 border-slate-200 text-slate-600 hover:bg-rose-50 hover:text-rose-600 shadow-sm"
          )}
        >
          登出
        </button>
