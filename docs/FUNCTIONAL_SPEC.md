...
7. 建立帳號
8. 自動登入
9. 跳轉到 /map（或原本的 returnUrl，若有）

#### 狀態變換
```
[未登入] → [註冊頁面] → [驗證中] → [建立帳號] → [已登入] → [/map 地圖頁]
```
...
4. 系統驗證憑證
5. 登入成功
6. 跳轉到 /map（或原本的 returnUrl，若有）

#### 狀態變換
```
[未登入] → [登入頁面] → [驗證中] → [已登入] → [/map 地圖頁]
```
...
> **術語說明**：Dot（地點）是 TravelDot 的核心資料單位，可獨立存在，不一定屬於任何 Trip。Trip（旅程）是 Dot 的分類工具，非必要。使用者建立地點記錄後，可選擇將其歸屬於某個旅程，或保持獨立。

### 2.1 建立新旅程

#### 操作流程
1. 使用者在 Profile 頁面（/profile）點擊「+ 新增旅程」
2. 彈出「建立旅程」Modal
3. 輸入旅程名稱、開始日期、結束日期
4. （選填）上傳封面照片
5. 點擊「建立」按鈕
...
#### 狀態變換
```
[Profile 頁面] → [建立旅程 Modal] → [驗證中] → [建立中] → [建立成功] → [更新列表]
```
...
#### 狀態變換
```
[Profile 頁面] → [右鍵選單] → [編輯 Modal] → [驗證中] → [更新中] → [更新成功] → [更新 UI]
```
...
#### 狀態變換
```
[Profile 頁面] → [右鍵選單] → [確認對話框] → [刪除中] → [刪除成功] → [更新 UI]
```
...
#### 狀態變換
```
[Profile 頁面（點擊旅程）] → [/map 載入中] → [地圖顯示] → [地點標記顯示] → [自動調整視野]
```
...
#### 操作流程
1. 在 Profile 頁面（/profile）的「資料管理」區塊點擊「匯出資料」
2. 顯示確認 Modal（預估檔案大小）
3. 點擊「開始匯出」
4. 顯示進度條
5. 產生 ZIP 檔案
...
#### 狀態變換
```
[Profile 頁面 (/profile)] → [確認 Modal] → [匯出中] → [產生 ZIP] → [下載] → [完成]
```
...
#### 功能說明
個人中心（/profile）整合了用戶基本資料、旅程管理清單、資料匯出入口與登出功能，取代原本分散在「Landing Page」與「設定頁」的功能。

#### 操作流程
1. 使用者訪問 /profile（需登入，未登入跳轉 /login）
2. 系統載入用戶資料（Email、加入日期）
3. 系統載入旅程列表（依建立時間倒序）
4. 使用者可點擊旅程卡片跳轉地圖並選取旅程
5. 使用者可點擊「匯出資料」啟動匯出流程
6. 使用者可點擊「登出」清除登入狀態

#### 狀態變換
```
[訪問 /profile] → [驗證登入狀態] → [載入用戶資料] → [顯示個人中心]
```
...
Then:
  - 「我的旅程」區塊顯示空狀態
  - 提示訊息: "還沒有旅程，前往地圖建立你的第一個旅程吧！"
  - 提供「前往地圖」連結按鈕
```

**Case 2: 點擊旅程後跳轉地圖**
```
Given: 使用者在 /profile 查看旅程列表
When: 點擊「泰國之旅」旅程卡片
Then:
  - tripStore.setCurrentTrip() 設定選取的旅程
  - 跳轉至 /map
  - 地圖顯示「泰國之旅」的所有地點
```
...
When: 點擊「登出」按鈕
Then:
  - 彈出確認提示（或直接執行，依 UX 決策）
  - 呼叫 Firebase signOut()
  - 清除 authStore 狀態
  - 清除 tripStore 與 mapStore 狀態
  - 跳轉至 /home
  - 顯示 Toast: "已成功登出"
```
...
```
Given: 使用者在 /profile
When: 點擊「刪除帳號」按鈕（如未來新增此功能）
Then:
  - 彈出確認對話框:
    - 標題: "確定要刪除帳號嗎？"
    - 說明: "此操作無法復原，所有旅程與地點將永久刪除"
    - 需要輸入 Email 確認
    - 按鈕: [取消] [確定刪除]（紅色）
  - 確認後刪除所有資料並跳轉 /home
```
