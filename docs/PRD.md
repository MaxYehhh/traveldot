...
**Dot（地點）** 是 TravelDot 的核心資料單位，代表一筆地理位置記錄，包含名稱、座標、照片、心得筆記等內容。Dot 可以獨立存在，不一定屬於任何 Trip。

**Trip（旅程）** 是 Dot 的分類容器，幫助使用者整理多個相關 Dot（例如：同一趟旅行的所有地點）。Trip 是選用的，使用者可以建立多個 Trip 來分類管理 Dot。

**術語對照**：UI 介面顯示「地點」（Dot）與「旅程」（Trip）；程式碼層維持 `place` 與 `trip` 變數名稱不變。
...
- Email 格式驗證（顯示即時錯誤訊息）
- 密碼強度驗證（至少 8 個字元）
- 註冊成功後自動登入並跳轉到 /map 地圖頁（或原 returnUrl）
- 顯示歡迎 Toast 訊息
...
```
As a 使用者
I want to 建立多個旅程來分類我的地點記錄
So that 我可以依照不同旅行主題整理我的 Dot
```
**Acceptance Criteria:**
- User 可建立多個 Trip
- Trip 作為 Dot 的分類容器，使用者可切換不同 Trip 查看對應地點
- 無 Trip 的情況下，使用者仍可使用地圖記錄 Dot
...
```
As a 使用者
I want to 將 Dot 加入或移出 Trip
So that 我可以彈性地重新分類地點記錄
```
**Acceptance Criteria:**
- Dot 可加入指定的 Trip（歸屬於該旅程）
- Dot 可從 Trip 中移出（成為無旅程的獨立 Dot）
- 移入/移出操作不刪除 Dot 本身的資料
...
```
As a 未登入訪客
I want to 在首頁了解 TravelDot 的核心價值
So that 我可以決定是否要註冊使用
```
**Acceptance Criteria:**
- 顯示品牌 Hero Section（大標題「Every Dot Tells a Story」+ 副標 + CTA 按鈕）
- 顯示 3 個功能亮點卡片（地圖記錄、照片串聯、旅程整理）
- 未登入訪客點擊「立即開始」CTA 跳轉至 /register
- 已登入用戶訪問 /home 時，顯示「前往地圖」捷徑（不強制跳轉）
- Nav：Logo 左、登入/註冊按鈕右（未登入）；或「前往地圖」按鈕（已登入）
...
```
As a 已登入使用者
I want to 在個人中心查看我的帳號資訊與旅程列表
So that 我可以管理我的資料並快速切換旅程
```
**Acceptance Criteria:**
- 顯示用戶 Email 與加入日期
- 顯示旅程列表（名稱 + 地點數 + 日期範圍）
...
**頁面定位說明（澄清）：**
- `/home` 是官方行銷形象頁（Epic 7），供未登入訪客了解產品，MVP 階段新增
- 旅程管理入口（建立/編輯/刪除旅程、切換旅程）移至 `/profile` 個人中心（Epic 8）
- 原先文件中提及的「Landing Page 旅程列表」現在統一稱為 `/profile` 個人中心的功能
