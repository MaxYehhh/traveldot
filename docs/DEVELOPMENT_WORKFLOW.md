...
在閱讀規格文件或撰寫程式碼時，請依以下對照表理解各術語的使用語境：
...
- **程式碼層**：變數名稱、函式名稱、Store 狀態名稱維持 `place` / `trip`（不更改現有程式碼）
- **UI 顯示層**：所有使用者可見的文字使用中文「地點」/「旅程」
- **概念/行銷層**：品牌文案、Hero Section、功能說明使用英文 Dot / Trip

**層級關係**：User → Trips（選用分類容器）→ Dots（核心資料單位，可獨立存在）
